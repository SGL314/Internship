...
# Garante que o diretório 'content' existe
import os
...
# Adiciona stopwords específicas conforme seu código original
...
qt_palavras = 100 # Esta variável controla o número de itens exibidos nas listas.
...
try:
    ...
    print(f"Erro: Não foi possível abrir '{txt_file_path_full}'. Certifique-se de que a conversão do PDF foi bem-sucedida e o arquivo existe.")
    filecontent = "" # Inicializa vazio para evitar erros posteriores


print(f"Tamanho do conteúdo: {len(filecontent)}")
...
"""## Pré-processamento dos elementos tokenizados
A ideia aqui é retirar todas as palavras que pertencem à lista de stopwords, deixar tudo em minúsculas, retirar espaços e quebras de linhas adicionais desnecessários.
"""
...
for sent_tokenizada in sentencas_tokenizadas:
    ...
    # Remove pontuação, caracteres específicos e números
    raw = [''.join(c for c in s if c not in remove_pt + '–' + '🙁' + '\’' + '\”' + '“') for s in raw]
    raw = [re.sub(r"\d+[.,]?\d*", "", s) for s in raw] # Remove números
    raw = [s for s in raw if s not in stop_words_pt] # Remove stopwords
    raw = [' '.join(s.split()) for s in raw if s] # Remove espaços extras e mantém strings não vazias
    string_processed = ' '.join(raw).strip() # Usa strip() para remover espaços no início e fim
    if string_processed: # Adiciona apenas se não for uma string vazia
        sent_tokenizada_preprocessed.append(string_processed)

"""# N-gramas"""

ngram_value = 2 # Definido como 2 para bigramas
most_common_value = qt_palavras # Número dos bigramas mais comuns a serem mostrados
...
formatted_common = [("_".join(bigram), count) for bigram, count in common] # retira os que não são bigramas

df_common = pd.DataFrame(formatted_common, columns=['Bigrama', 'Frequência'])

# Exibe apenas os bigramas e suas frequências
print("\n--- Bigramas e suas Frequências ---")
print(df_common)
...
plt.axis("off")
plt.box(False)
plt.title("Nuvem de Palavras dos Bigramas")
plt.show()